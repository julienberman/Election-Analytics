...
2. Create a three dimensional matrix to search for both alpha and lambda simultaneously. The first dimension is for each value of `\(\alpha\)` from `\(\alpha_{min}\)` to `\(\alpha_{max}\)`. The second dimension is for each value of `\(\lambda\)` from `\(\lambda_{min}\)` to `\(\lambda_{max}\)`. The third dimension is for each year I left out during cross validation. (I set `\(\alpha\)` to range between `\(0\)` and `\(1\)`, and I set `\(\lambda\)` to range between `\(0\)` and `\(5\)`.)
...
4. Calculate the average MSE for each pair ($\alpha$, `\(\lambda\)`). Average across the third dimension (the years) to get a 2D matrix of average MSEs for each combination of `\(\alpha\)` and `\(\lambda\)`.
