...
train_elastic_net_fast <- function(data, formula, n_bootstrap = 1000, min_obs_per_fold = 3) {
  ...
  boot_results <- boot(data = data, statistic = boot_func, R = n_bootstrap)
  
  # Calculate standard errors, handling NAs
  se <- apply(boot_results$t, 2, function(x) sd(x, na.rm = TRUE))
...
# simulate election
simulate_election <- function(nat_polls_model_info, state_model_info, national_data_test, state_data_test, n_simulations = 10000) {
