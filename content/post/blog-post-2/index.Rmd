...
write_csv(df_full_merged, "merged.csv")
...
econ_model <- lm(margin_nat * incumb ~ jobs_agg + pce_agg + rdpi_agg +
...
```
```{r ex_incl_grap_1, out.width = "50%", echo = FALSE, eval=TRUE}
...
for (year in years) {
  ...
  model <- lm(margin_nat*incumb ~ jobs_agg + pce_agg + rdpi_agg + cpi_agg +
...
mse_out <- mean((predictions - actuals)^2) %>% print()
...
plot(df_collapsed$year, 
     ...
     xlab = "Year", 
     ylab = "Vote Margin", 
     main = "True Margin (Line) vs Predicted Margin (Dots) for Each Election Cycle") +
